...
test('open reuses only live project connections without extending idle expiry or reviving old URLs',async()=>{
  ...
  try{
    ...
    assert.equal(a.connectionName,'WebGPT Worker');assert.equal(a.connectionUrl,undefined);
    const attachment=readFileSync(a.attachmentPath,'utf8');
    assert.ok(attachment.includes(realpathSync(dir)));assert.ok(attachment.includes('Task token: '));
    assert.ok(Buffer.byteLength(attachment)<1024);
    const token=attachment.match(/^Task token: (.+)$/m)[1];
    const inspect=async value=>(await (await fetch(`http://127.0.0.1:${service.mcpPort}/mcp`,{method:'POST',body:JSON.stringify({jsonrpc:'2.0',id:1,method:'tools/call',params:{name:'get_task',arguments:{token:value}}})})).json()).result;
    assert.equal((await inspect(token)).structuredContent.mode,'open');
    assert.equal((await inspect(token)).structuredContent.terminal.cwd,realpathSync(dir));
    clock+=1000;
    ...
    assert.equal(changedOrigin.id,a.id);assert.equal(changedOrigin.connectionName,a.connectionName);
    assert.equal(changedOrigin.attachmentPath,a.attachmentPath);
    ...
    assert.equal(c.reused,false);assert.notEqual(c.attachmentPath,a.attachmentPath);
    assert.equal(c.connectionName,a.connectionName);
    assert.notEqual(readFileSync(c.attachmentPath,'utf8'),attachment);
    assert.equal((await inspect(token)).isError,true);
...
test('open connection binds its project without prompt tokens and exposes only terminal tools',async()=>{
  ...
  try{
    ...
    assert.ok(readFileSync(cli.attachmentPath,'utf8').includes('Task token: '));assert.equal(cli.token,undefined);
